...
dev-install: dune-install
	install -m 555 $(OPAM_SWITCH_PREFIX)/bin/pdfs ~/bin
.PHONY: home-install
...
publish: build mounts
	echo 'url { src:' $$(cat pdfs.opam | grep dev-repo | awk '{ print $$2 }') '}' >> pdfs.opam
	make -C /data/web/dldc/opam add NAME=pdfs OPAM=$$PWD/pdfs.opam
.PHONY: publish
